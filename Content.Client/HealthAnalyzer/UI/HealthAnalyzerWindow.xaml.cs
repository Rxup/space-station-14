...
namespace Content.Client.HealthAnalyzer.UI
{
    ...
    public sealed partial class HealthAnalyzerWindow : FancyWindow
    {
        ...
        public void Populate(HealthAnalyzerScannedUserMessage msg)
        {
            ...
            HungerLabel.Text = !float.IsNaN(msg.HungerLevel)
                ? $"{msg.HungerLevel * 100:F1} %"
                : Loc.GetString("health-analyzer-window-entity-unknown-value-text");

            ThirstLabel.Text = !float.IsNaN(msg.ThirstLevel)
                ? $"{msg.ThirstLevel * 100:F1} %"
                : Loc.GetString("health-analyzer-window-entity-unknown-value-text");
            ...
            if (msg.HungerAlert is { } hungerAlert)
            {
                var hungerLabel = new RichTextLabel
                {
                    Margin = new Thickness(0, 4),
                    MaxWidth = 300,
                };
                var hungerKey = hungerAlert == HungerThreshold.Starving
                    ? "health-analyzer-window-patient-starving"
                    : "health-analyzer-window-patient-peckish";
                var hungerColor = hungerAlert == HungerThreshold.Starving ? Color.Red : Color.Orange;
                hungerLabel.SetMessage(Loc.GetString(hungerKey), defaultColor: hungerColor);
                AlertsContainer.AddChild(hungerLabel);
            }

            if (msg.ThirstAlert is { } thirstAlert)
            {
                var thirstLabel = new RichTextLabel
                {
                    Margin = new Thickness(0, 4),
                    MaxWidth = 300,
                };
                var thirstKey = thirstAlert == ThirstThreshold.Parched
                    ? "health-analyzer-window-patient-parched"
                    : "health-analyzer-window-patient-thirsty";
                var thirstColor = thirstAlert == ThirstThreshold.Parched ? Color.Red : Color.Orange;
                thirstLabel.SetMessage(Loc.GetString(thirstKey), defaultColor: thirstColor);
                AlertsContainer.AddChild(thirstLabel);
            }
