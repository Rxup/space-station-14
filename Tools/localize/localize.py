...
class LocalizationManager:
    ...
    def _load_config(self, config_file: str) -> Tuple[List[Path], List[Path]]:
        ...
        # start-backmen: localize-config-path
        if not config_path.exists():
            config_path = (
                Path(__file__).resolve().parent / Path(config_file).name
            ).resolve()
        # end-backmen: localize-config-path
    ...
    def _generate_prototype_ftls(
        ...
        for proto in prototypes.values():
            ...
            if mem_entry:
                # start-backmen: preserve-proto-ftl-translations
                # Keep existing human translations even when YAML inherits name/desc
                # from a parent (proto.name / proto.desc is None). Otherwise
                # --prototypes rewrites them to "{ ent-Parent }" and drops ru text.
                ...
                for attr_name, mem_attr in mem_entry.attributes.items():
                    if not mem_attr.value.strip() or mem_attr.value.startswith("{"):
                        continue
                ...
                # end-backmen: preserve-proto-ftl-translations
