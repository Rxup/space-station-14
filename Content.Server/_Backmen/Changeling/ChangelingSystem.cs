...
using System.Linq;
using Content.Server._Backmen.Objectives.Components;
using Content.Shared._Backmen.Changeling;
using Content.Shared._Backmen.Changeling.Components;
...
namespace Content.Server._Backmen.Changeling;
        ...
        if (comp.Biomass <= comp.MaxBiomass / 3)
        {
            ...
            if (random == 1)
            {
                if (HasComp<StatusEffectsComponent>(uid))
                    _stun.TryUpdateStunDuration(uid, TimeSpan.FromSeconds(1.5f));
