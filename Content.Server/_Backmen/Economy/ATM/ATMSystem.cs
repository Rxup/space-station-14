...
using Content.Shared._Backmen.Economy.ATM;
...
namespace Content.Server._Backmen.Economy.ATM;
...
public sealed class ATMSystem : SharedATMSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<AtmComponent, EntInsertedIntoContainerMessage>((Entity<AtmComponent> uid,
            ref EntInsertedIntoContainerMessage arg) => UpdateComponentUserInterface(uid));
        SubscribeLocalEvent<AtmComponent, EntRemovedFromContainerMessage>((Entity<AtmComponent> uid,
            ref EntRemovedFromContainerMessage _) => UpdateComponentUserInterface(uid));
        ...
        SubscribeLocalEvent<AtmCurrencyComponent, AfterInteractEvent>(OnAfterInteract,
    ...
    private void OnAfterInteract(EntityUid uid, AtmCurrencyComponent _, AfterInteractEvent args)
    {
        ...
        {
            args.Handled = TryAddCurrency(GetCurrencyValue(args.Used, component),
                (args.Target.Value, atmComponent),
                args.User);
        }
        else if (TryComp<StoreComponent>(args.Target, out var store))
        {
            args.Handled =
                _storeSystem.TryAddCurrency(GetCurrencyValueOld(args.Used, component), args.Target.Value, store);
        }
    ...
    public void UpdateUi(EntityUid uid, BankAccountComponent bankAccount)
    {
        ...
        var currencySymbol = "";
        if (_proto.TryIndex(bankAccount.CurrencyType, out CurrencyPrototype? p))
    ...
    private void UpdateComponentUserInterface(Entity<AtmComponent> uid, EntityUid? actor = null)
    {
        ...
        if (uid.Comp.IdCardSlot.Item is { Valid: true } idCardEntityUid)
        {
            if (_entities.TryGetComponent<IdCardComponent>(idCardEntityUid, out var idCardComponent))
            {
                ...
                if (!_bankManagerSystem.TryGetBankAccount(idCardEntityUid,
                        out var bankAccount)) // новая карта (заведение счета в банке)
                {
                ...
                if (_proto.TryIndex(bankAccount.Value.Comp.CurrencyType, out CurrencyPrototype? p))
                    currencySymbol = Loc.GetString(p.CurrencySymbol);
            }
    ...
    public bool TryAddCurrency(Dictionary<ProtoId<CurrencyPrototype>, FixedPoint2> currency,
    ...
    private bool TryGetBankAccountNumberFromStoredIdCard(Entity<AtmComponent> component,
        [NotNullWhen(true)] out Entity<BankAccountComponent>? storedBankAccountNumber)
    {
