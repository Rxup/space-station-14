using System.Linq;
using Content.Server._Backmen.Disease.Components;
using Content.Server._Backmen.Disease.Cures;
using Content.Server.Body.Components;
using Content.Server.Chat.Systems;
using Content.Server.Popups;
using Content.Shared._Backmen.CCVar;
using Content.Shared._Backmen.Disease;
using Content.Shared._Backmen.Disease.Events;
...
namespace Content.Server._Backmen.Disease;
...
public sealed class DiseaseSystem : SharedDiseaseSystem
{
    ...
    private readonly HashSet<(Entity<DiseaseCarrierComponent> carrier, ProtoId<DiseasePrototype> disease)> _cureQueue =
        new();
    ...
    public void TryMetabolize(Entity<DiseaseCarrierComponent?> ent, ReagentId reagentId, FixedPoint2 amount)
    {
        ...
        foreach (var disease in ent.Comp.Diseases)
        {
            ...
            {
                if (!cure.Stages.Contains(stage) || cure is not DiseaseReagentCure cureReagent ||
                    cureReagent.Reagent != reagentId)
                    continue;
    ...
    public override void Update(float frameTime)
    {
        ...
        foreach (var (carrier, disease) in _cureQueue)
        {
            ...
            if (carrier.Comp.Diseases.Count ==
                0) //This is reliable unlike testing Count == 0 right after removal for reasons I don't quite get
...
*/
    ...
    private void OnAfterInteract(EntityUid uid, DiseaseVaccineComponent vaxx, AfterInteractEvent args)
    {
        ...
        var doAfterArgs =
            new DoAfterArgs(EntityManager, args.User, vaxx.InjectDelay, ev, uid, target: args.Target, used: uid)
            {
                BreakOnMove = true,
                NeedHand = true
            };
    ...
    private void OnApplyMetabolicMultiplier(EntityUid uid,
        DiseaseCarrierComponent component,
    ...
    /// </summary>
    private void InteractWithDiseased(EntityUid diseased,
        EntityUid target,
        DiseaseCarrierComponent? diseasedCarrier = null)
    {
    ...
    /// for TryInfect.
    /// </summary>
    public override void TryAddDisease(EntityUid host,
        DiseasePrototype addedDisease,
    ...
    public void TryInfect(Entity<DiseaseCarrierComponent> carrier,
    ...
    /// </summary>
    public bool SneezeCough(EntityUid uid,
        ProtoId<DiseasePrototype> diseaseId,
        string emoteId,
        bool airTransmit = true,
