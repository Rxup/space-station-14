...
public sealed partial class NPCSteeringSystem
{
    ...
    private SteeringObstacleStatus TryHandleFlags(EntityUid uid, NPCSteeringComponent component, PathPoly poly)
    {
            ...
            (poly.Data.CollisionMask & layer) != 0x0)
        {
            ...
            {
                if (_melee.TryGetWeapon(uid, out var meleeWeaponUid , out var meleeWeapon) && meleeWeapon.NextAttack <= _timing.CurTime && TryComp<CombatModeComponent>(uid, out var combatMode))
                {
                    ...
                    foreach (var ent in obstacleEnts)
                    {
                        ...
                        {
                            attackResult = _melee.AttemptLightAttack(uid, meleeWeaponUid, meleeWeapon, ent);
                            break;
                        }
                    }
