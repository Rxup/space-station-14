...
public sealed class MetabolizerSystem : SharedMetabolizerSystem
{
    ...
    private void TryMetabolize(Entity<MetabolizerComponent, OrganComponent?, SolutionContainerManagerComponent?> ent)
    {
        ...
        foreach (var (reagent, quantity) in list)
        {
            ...
            var actualEntity = ent.Comp2?.Body ?? solutionEntityUid.Value; // backmen
            ...
            var removed = FixedPoint2.Zero;// backmen
            // remove a certain amount of reagent
            if (mostToRemove > FixedPoint2.Zero)
            {
                removed = solution.RemoveReagent(reagent, mostToRemove); // backmen
            ...
            {
                var ev2 = new ReagentMetabolised(reagent, removed);
                RaiseLocalEvent(actualEntity, ref ev2);
            }
            // end-backmen
        }
