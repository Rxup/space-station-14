...
public sealed class SetBankCurrencyCommand : IConsoleCommand
{
    ...
    public void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        {
            shell.WriteError($"Добавить на счет не удалось!");
            return;
        }

        _adminLogger.Add(LogType.AdminMessage, LogImpact.Extreme,
            $"Admin {(shell.Player != null ? shell.Player.Name : "An administrator")} SetBankCurrency {account.AccountName} #{account.AccountNumber} balance: {account.Balance}");
    }
