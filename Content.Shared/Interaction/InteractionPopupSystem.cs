        ...
        EntityUid user)
    {
        ...
        if (_random.Prob(component.SuccessChance))
        {
            if (component.InteractSuccessString != null)
            {
                ...
                {
                    var hug = new MoodEffectEvent("BeingHugged"); // backmen: mood
                    RaiseLocalEvent(target, hug);
                }
                else if (component.InteractSuccessString.Contains("petting-success-"))
                {
                    var pet = new MoodEffectEvent("PetAnimal"); // backmen: mood
                    RaiseLocalEvent(user, pet);
                }
            }
