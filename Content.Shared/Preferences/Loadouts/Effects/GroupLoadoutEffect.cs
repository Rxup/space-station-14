...
public sealed partial class GroupLoadoutEffect : LoadoutEffect
{
    ...
    public override bool Validate(HumanoidCharacterProfile profile, RoleLoadout loadout, LoadoutPrototype proto/*Corvax-Sponsors*/, ICommonSession session, IDependencyCollection collection, [NotNullWhen(false)] out FormattedMessage? reason)
    {
        var effectsProto = collection.Resolve<IPrototypeManager>().Index(Proto);

        foreach (var effect in effectsProto.Effects)
        {
            if (!effect.Validate(profile, proto, loadout, session, collection, out reason))
                return false;
        }
