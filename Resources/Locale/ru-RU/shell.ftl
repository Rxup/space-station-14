...
shell-entity-with-uid-lacks-component = Сущность с uid { $uid } не имеет {INDEFINITE($componentName)} компонента { $componentName }.
shell-entity-target-lacks-component = Целевая сущность не имеет {INDEFINITE($componentName)} компонента { $componentName }.
shell-invalid-color-hex = Недопустимый HEX-цвет!
shell-target-player-does-not-exist = Целевой игрок не существует!
shell-target-entity-does-not-have-message = У целевой сущности нет {INDEFINITE($missing)} { $missing }!
